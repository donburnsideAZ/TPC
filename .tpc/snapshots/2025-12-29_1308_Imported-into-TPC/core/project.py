...
OLD_KNOWN_PROJECTS_FILE = OLD_CONFIG_DIR / "known_projects.json"
...
def migrate_global_config() -> bool:
    ...
    This handles the one-time switch from ptc to tpc branding.
    Returns True if migration occurred.
    """
    # Skip if new config already exists or old config doesn't exist
...
def migrate_project_config(project_path: Path) -> bool:
    ...
    # Skip if new config exists or old config doesn't exist
...
def cleanup_stale_projects() -> tuple[int, list[str]]:
    ...
    for path in paths:
        ...
        if not path.exists():
            ...
            # Folder exists but check for .tpc config
            tpc_config = path / ".tpc" / "project.json"
            ptc_config = path / ".ptc" / "project.json"  # Also check old style
...
def get_orphan_folders(root: Optional[Path] = None) -> list[Path]:
    ...
    - Has no .tpc or .ptc config folder
...
class Project:
    ...
    def load(cls, path: Path) -> "Project":
        ...
        # Migrate from old .ptc folder if needed
...
''')
    ...
    def import_existing(
        ...
        # Check if already a TPC project (including old .ptc folders)
        if (folder / ".tpc" / "project.json").exists():
            raise ValueError("This folder is already a TPC project")
        if (folder / ".ptc" / "project.json").exists():
            raise ValueError("This folder is already a TPC project (needs migration)")
...
def find_tpc_projects(root: Optional[Path] = None) -> list[Project]:
    ...
    Also finds old .ptc folders and migrates them automatically.
    
    TPC 2.0: Uses the configured projects_root from config.json
    instead of hardcoded DEFAULT_PROJECTS_ROOT.
    """
    if root is None:
        # TPC 2.0: Use config's projects_root if available
        try:
    ...
    # Search the default/provided root for both .tpc and .ptc folders
    if root.exists():
        # Find .tpc folders (new style)
        ...
        # Find .ptc folders (old style) - will be migrated by Project.load()
        for ptc_dir in root.rglob(".ptc"):
            if ptc_dir.is_dir() and (ptc_dir / "project.json").exists():
                project_path = ptc_dir.parent
                if project_path not in seen_paths:
                    try:
                        project = Project.load(project_path)  # This migrates .ptc → .tpc
                        projects.append(project)
                        seen_paths.add(project_path)
                    except Exception:
                        pass  # Skip invalid projects
    ...
    for project_path in get_known_project_paths():
        if project_path not in seen_paths and project_path.exists():
            # Check for either .tpc or .ptc (Project.load handles migration)
