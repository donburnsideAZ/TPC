...
Handles:
- Personal Access Token storage (secure keychain via keyring)
- Credential injection for git operations
- Token validation
- Clone operations

SECURITY: Tokens are stored in the system keychain, NOT in plaintext files.
- macOS: Keychain Access (encrypted, requires user auth)
...
# Try to import keyring, with fallback messaging
try:
    import keyring
...
# Keyring service identifier - used to identify TPC credentials in system keychain
KEYRING_SERVICE = "TPC-GitHub"

# TPC config location (for non-sensitive settings only)
...
def _migrate_plaintext_token() -> None:
    """
    One-time migration: Move plaintext token to keychain.
    
    If we find a github_token in config.json, move it to the secure
    keychain and remove it from the plaintext file.
    """
    if not KEYRING_AVAILABLE:
        return
    
    config = load_config()
    plaintext_token = config.get("github_token")
    username = config.get("github_username")
    
    if plaintext_token and username:
        try:
            # Save to keychain
            keyring.set_password(KEYRING_SERVICE, username, plaintext_token)
            
            # Remove from config file
            config.pop("github_token", None)
            save_config(config)
            
            print(f"[TPC] Migrated GitHub token to secure keychain storage")
        except Exception as e:
            # Don't fail on migration error - user can still authenticate
            print(f"[TPC] Warning: Could not migrate token to keychain: {e}")


def get_github_token() -> Optional[str]:
    """
    Get the stored GitHub Personal Access Token from the system keychain.
    
    Returns None if not stored or keyring not available.
    """
    ...
    if not KEYRING_AVAILABLE:
        # Fallback: check config file for legacy plaintext storage
        # This should only happen if keyring couldn't be installed
        config = load_config()
...
def get_github_username() -> Optional[str]:
    ...
    Username is stored in config.json (not sensitive).
    """
    config = load_config()
...
def save_github_credentials(username: str, token: str) -> None:
    ...
    - Username: Stored in config.json (not sensitive)
    - Token: Stored in system keychain (encrypted)
    """
    # Save username in config (not secret)
    config = load_config()
    config["github_username"] = username
    
    # Remove any legacy plaintext token
    config.pop("github_token", None)
    
    save_config(config)
    
    # Save token in system keychain (encrypted)
    if KEYRING_AVAILABLE:
        try:
            keyring.set_password(KEYRING_SERVICE, username, token)
        except Exception as e:
            # If keychain fails, fall back to config file with warning
            print(f"[TPC] Warning: Could not save to keychain ({e}), using config file")
            config["github_token"] = token
            save_config(config)
    else:
        # No keyring available - use config file with warning
        print("[TPC] Warning: keyring not available, storing token in config file")
        config["github_token"] = token
        save_config(config)
...
def clear_github_credentials() -> None:
    ...
    # Remove from keychain if we have a username
    if username and KEYRING_AVAILABLE:
        try:
            keyring.delete_password(KEYRING_SERVICE, username)
        except keyring.errors.PasswordDeleteError:
            pass  # Password didn't exist, that's fine
        except Exception:
            pass  # Other keychain errors, continue cleanup
    
    # Remove from config file
    config = load_config()
    config.pop("github_username", None)
    config.pop("github_token", None)  # Remove any legacy plaintext token
    save_config(config)
...
def is_keyring_available() -> bool:
    """Check if secure keychain storage is available."""
...
# This will automatically move any existing plaintext tokens to the keychain
