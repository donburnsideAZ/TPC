...
from core import Project, find_tpc_projects, invalidate_project_cache, DEFAULT_PROJECTS_ROOT, remove_from_tpc


class ProjectLoaderThread(QThread):
    """Background thread for loading projects without blocking the UI."""

    projects_loaded = pyqtSignal(list)  # Emits list of Project objects

    def run(self):
        # Force fresh scan (bypass cache for initial load)
        projects = find_tpc_projects(use_cache=False)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._loader_thread: ProjectLoaderThread | None = None

        # Build the UI
        self.setup_ui()

        # Load existing projects in background
    ...
    def _start_background_load(self):
        """Start loading projects in a background thread."""
        # Show loading indicator
        self.project_list.clear()
        loading_item = QListWidgetItem("Loading projects...")
        loading_item.setFlags(Qt.ItemFlag.NoItemFlags)  # Not selectable
        self.project_list.addItem(loading_item)

        # Start background thread
        self._loader_thread = ProjectLoaderThread()
        self._loader_thread.projects_loaded.connect(self._on_projects_loaded)
        self._loader_thread.start()

    def _on_projects_loaded(self, projects: list):
        """Handle projects loaded from background thread."""
        self.project_list.clear()
    ...
    def refresh_project_list(self):
        """Reload the project list from disk (uses cache for fast refresh)."""
        # For manual refreshes after project changes, use cache (fast)
        self.project_list.clear()

        projects = find_tpc_projects(use_cache=True)
    ...
    def on_new_project(self):
        ...
        if wizard.exec():
            ...
            if project:
                invalidate_project_cache()  # Force rescan
                self.refresh_project_list()
                self.current_project = project
                self.workspace_widget.set_project(project)
                self.workspace_stack.setCurrentWidget(self.workspace_widget)
