...
from core import Project, __version__
...
class PackWorkspace(QWidget):
    ...
    def setup_ui(self):
        ...
        # AI Build Help button (at top, matching Track workspace style)
        self.btn_ai_build_help = QPushButton("📋 Copy AI Build Help Prompt")
        self.btn_ai_build_help.setObjectName("utilityButton")
        self.btn_ai_build_help.setToolTip("Copy a prompt with build details to paste into ChatGPT, Claude, etc.")
        self.btn_ai_build_help.clicked.connect(self.on_copy_ai_build_help)
        right_layout.addWidget(self.btn_ai_build_help)
    ...
    def _check_existing_icon(self):
        ...
        # First, check if we have a persisted icon path from previous session
        if hasattr(self.project, 'icon_path') and self.project.icon_path:
            persisted_path = Path(self.project.icon_path)
            if persisted_path.exists():
                self._set_icon(persisted_path, save_to_config=False)  # Don't re-save what we just loaded
                return
        
        # Fall back to scanning for common icon locations
        icon_patterns = [
    ...
    def _set_icon(self, icon_path: Path, save_to_config: bool = True):
        """Set the icon from a file path and show preview.
        
        Args:
            icon_path: Path to the icon file
            save_to_config: Whether to persist the path to project config (default True)
        """
        ...
        # Persist to project config so it's remembered next time
        if save_to_config and self.project and hasattr(self.project, 'icon_path'):
            self.project.icon_path = str(icon_path)
            self.project.save_config()
        
        # For size analysis, check if there's a source PNG when loading a converted ICO/ICNS
        # This prevents showing "256x256" warning when the source was actually 512x512
        analysis_path = icon_path
        if icon_path.suffix.lower() in ('.ico', '.icns'):
            # Look for matching PNG source
            png_source = icon_path.with_suffix('.png')
            if png_source.exists():
                analysis_path = png_source
        
        # Analyze the image (use source PNG if available for accurate size reporting)
        info = self.icon_alchemist.analyze_image(analysis_path)
        
        # Show preview (use source PNG if available - Qt loads 16x16 from ICO by default)
        pixmap = QPixmap(str(analysis_path))
    ...
    def _convert_to_ico(self, source_path: Path):
        ...
        if result.success:
            ...
            # Persist the converted icon path
            if self.project and hasattr(self.project, 'icon_path'):
                self.project.icon_path = str(result.output_path)
                self.project.save_config()
    ...
    def on_copy_ai_build_help(self):
        """Copy a context-aware AI help prompt for build issues to clipboard."""
        if not self.project:
            QMessageBox.information(
                self, 
                "No Project", 
        ...
        wrangler = EnvironmentWrangler()
        venv_info = {
            "exists": wrangler.venv_exists(self.project.name),
            "packages": wrangler.get_installed_packages(self.project.name) if wrangler.venv_exists(self.project.name) else [],
            "size": wrangler.format_size(wrangler.get_venv_size(self.project.name)) if wrangler.venv_exists(self.project.name) else "N/A"
        }
        
        # Get current output (build output might be in main workspace)
        last_output = ""  # Pack workspace doesn't have its own output panel
        
        # Generate the prompt
        prompt = self._generate_pack_help_prompt(venv_info)
        ...
        QMessageBox.information(
            self,
            "Copied!",
            "AI build help prompt copied to clipboard.\n\n"
            "Paste it into ChatGPT, Claude, or any AI assistant,\n"
            "then describe your build issue at the end."
        )
    
    def _generate_pack_help_prompt(self, venv_info: dict) -> str:
        ...
        req_path = self.project.path / "requirements.txt" if self.project else None
        ...
        # Check for icon
        icon_path = self.project.icon_path if self.project and hasattr(self.project, 'icon_path') else None
...
- **Third-party imports found:** {len(self.scan_result.third_party)}
- **Packages:** {", ".join(sorted(self.scan_result.third_party)[:20])}{"..." if len(self.scan_result.third_party) > 20 else ""}
"""
...
- **Project:** {self.project.name if self.project else "None selected"}
- **Project Path:** {self.project.path if self.project else "N/A"}
- **Main File:** {self.project.main_file if self.project else "N/A"}

## Build Environment Status
- **Virtual environment exists:** {"Yes" if venv_exists else "No"}
- **Venv location:** `~/.tpc/venvs/{self.project.name if self.project else "ProjectName"}/`
- **Venv size:** {venv_size}
- **Packages installed:** {len(venv_packages)} ({", ".join(venv_packages[:10])}{"..." if len(venv_packages) > 10 else ""})
- **Icon configured:** {"Yes - " + str(icon_path) if icon_path else "No"}
{scan_info}
## requirements.txt
