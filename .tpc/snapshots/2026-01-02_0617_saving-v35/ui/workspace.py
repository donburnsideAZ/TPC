...
from PyQt6.QtCore import Qt, pyqtSignal, QProcess, QThread, QObject, QPoint
from PyQt6.QtGui import QFont, QTextCursor, QPixmap

from core import Project, Snapshot, __version__


def generate_track_help_prompt(project: Project, last_output: str = "") -> str:
    """Generate a context-aware help prompt for Track (project/versioning) issues."""
    ...
    # Get project info
    snapshot_count = len(project.get_version_history()) if project else 0
    has_github = project.has_github_backup() if project else False
    last_backup = project.get_last_backup_date() if project else None
    project_type = project.project_type if project else "unknown"
    
    # Format last backup date
    if last_backup:
        backup_str = last_backup.strftime("%Y-%m-%d %H:%M")
    else:
        backup_str = "Never"
    
    # Trim output to last 50 lines if too long
    output_lines = last_output.strip().split('\n') if last_output else []
    if len(output_lines) > 50:
        output_lines = output_lines[-50:]
        last_output = "...(truncated)...\n" + '\n'.join(output_lines)
    
    prompt = f"""I need help with TPC (Track Pack Click), a GUI app for version control and Python packaging.

## My Setup
- **TPC Version:** {__version__}
- **Platform:** {system} ({system_version})
- **Project:** {project.name if project else "None selected"}
- **Project Type:** {project_type}
- **Project Path:** {project.path if project else "N/A"}
- **Main File:** {project.main_file if project and project_type == "python" else "N/A"}

## Project Status
- **Snapshots saved:** {snapshot_count}
- **GitHub backup configured:** {"Yes" if has_github else "No"}
- **Last backup:** {backup_str}

## What TPC Does (so you understand the context)

TPC uses simple folder-based snapshots for version control (NOT Git commits):
- "Save Version" = Copy project files to `.tpc/snapshots/` folder
- "Restore" = Copy a snapshot back to the project folder
- "Backup to GitHub" = One-way push for offsite safety (optional)

**Key paths:**
- Project config: `ProjectFolder/.tpc/project.json`
- Snapshots: `ProjectFolder/.tpc/snapshots/`
- Global config: `~/.tpc/config.json`

## Important Guidelines

Please help me within TPC's mental model:
- Don't suggest Git commands (TPC doesn't use Git for versioning anymore)
- Don't suggest terminal commands unless absolutely necessary
- If suggesting file edits, tell me exactly which file and what to change

## Recent Output/Errors
```
{last_output if last_output else "(No recent output)"}
```
...
def generate_pack_help_prompt(project: Project, last_output: str = "", venv_info: dict = None) -> str:
    """Generate a context-aware help prompt for Pack (build) issues."""
    ...
    if venv_info is None:
        venv_info = {}
    venv_exists = venv_info.get("exists", False)
    venv_packages = venv_info.get("packages", [])
    venv_size = venv_info.get("size", "unknown")
    
    # Check for requirements.txt
    req_path = project.path / "requirements.txt" if project else None
    has_requirements = req_path.exists() if req_path else False
    requirements_content = ""
    if has_requirements:
        try:
            requirements_content = req_path.read_text()[:1000]  # First 1000 chars
        except:
            requirements_content = "(Could not read)"
    
    # Check for icon
    icon_path = project.icon_path if project and hasattr(project, 'icon_path') else None
    
    # Trim output to last 80 lines for build errors (often longer)
    output_lines = last_output.strip().split('\n') if last_output else []
    if len(output_lines) > 80:
        output_lines = output_lines[-80:]
        last_output = "...(truncated)...\n" + '\n'.join(output_lines)
    
    prompt = f"""I need help with TPC (Track Pack Click) Pack feature - building a standalone executable.
...
- **Project:** {project.name if project else "None selected"}
- **Project Path:** {project.path if project else "N/A"}
- **Main File:** {project.main_file if project else "N/A"}

## Build Environment Status
- **Virtual environment exists:** {"Yes" if venv_exists else "No"}
- **Venv location:** `~/.tpc/venvs/{project.name if project else "ProjectName"}/`
- **Venv size:** {venv_size}
- **Packages installed:** {len(venv_packages)} ({", ".join(venv_packages[:10])}{"..." if len(venv_packages) > 10 else ""})
- **Icon configured:** {"Yes - " + icon_path if icon_path else "No"}

## requirements.txt
{"```" + chr(10) + requirements_content + chr(10) + "```" if has_requirements else "(No requirements.txt found)"}

## What TPC Pack Does

TPC Pack builds standalone executables using PyInstaller:
1. Scans Python imports to detect dependencies
2. Creates isolated venv at `~/.tpc/venvs/ProjectName/`
3. Installs dependencies in that venv
4. Converts PNG icons to .ico (Windows) or .icns (Mac)
5. Runs PyInstaller with appropriate flags
6. Outputs to `ProjectFolder/TPC Builds/`

**Common issues:**
- Missing hidden imports (dynamic imports PyInstaller can't detect)
- Package name vs import name mismatches (e.g., `PIL` vs `Pillow`)
- Icon conversion problems
- Platform-specific dependencies

## Important Guidelines

- Don't suggest I run pip/pyinstaller directly - TPC manages the venv
- If a package is missing, tell me to add it to `requirements.txt`
- For hidden imports, explain what to add and where
- If suggesting file edits, be specific about paths

## Recent Build Output/Errors
```
{last_output if last_output else "(No recent output)"}
```
...
[Describe your build problem here]
"""
    return prompt
...
class WorkspaceWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        # Project icon (shows app icon if configured)
        ...
        self.project_icon.setScaledContents(False)
        self.project_icon.hide()  # Hidden until we have an icon
        header_layout.addWidget(self.project_icon)
        ...
        # AI Help button
        self.btn_ai_help = QPushButton("📋 Copy AI Help Prompt")
        self.btn_ai_help.setObjectName("utilityButton")
        self.btn_ai_help.setToolTip("Copy a prompt with your project details to paste into ChatGPT, Claude, etc.")
        self.btn_ai_help.clicked.connect(self.on_copy_ai_help)
        right_layout.addWidget(self.btn_ai_help)
    ...
    def _refresh_project_icon(self):
        """Load and display the project icon if available."""
        if not self.project:
            self.project_icon.hide()
            return
        
        icon_path = None
        
        # Check for configured icon in project
        if hasattr(self.project, 'icon_path') and self.project.icon_path:
            configured_path = Path(self.project.icon_path)
            if configured_path.exists():
                # Prefer PNG source for better quality display
                if configured_path.suffix.lower() in ('.ico', '.icns'):
                    png_source = configured_path.with_suffix('.png')
                    if png_source.exists():
                        icon_path = png_source
                    else:
                        icon_path = configured_path
                else:
                    icon_path = configured_path
        
        # Fall back to common icon locations if no configured icon
        if not icon_path:
            common_locations = [
    ...
    def on_copy_ai_help(self):
        """Copy a context-aware AI help prompt to clipboard."""
        if not self.project:
            QMessageBox.information(
                self, 
                "No Project", 
                "Select a project first to generate a help prompt."
            )
            return
        
        # Get current output panel content
        last_output = self.output_panel.toPlainText()
        
        # Generate the Track help prompt
        prompt = generate_track_help_prompt(self.project, last_output)
        ...
        QMessageBox.information(
            ...
            "then describe your issue at the end."
        )
