...
class WorkspaceWidget(QWidget):
    ...
    def on_merge_claude_branch(self):
        ...
        if success:
            ...
            self.project.mark_branch_handled(self._current_claude_branch, "merged")
            
            # Auto-commit so user doesn't see "unsaved changes" for a system action
            self.project.save_version("TPC: merged Claude branch")
    ...
    def on_ignore_claude_branches(self):
        ...
        if self._current_claude_branch and self.project:
            # Mark this branch as handled in project config
            self.project.mark_branch_handled(self._current_claude_branch, "ignored")
            
            # Auto-commit so user doesn't see "unsaved changes" for a system action
            self.project.save_version("TPC: dismissed Claude branch notification")
