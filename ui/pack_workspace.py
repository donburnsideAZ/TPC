...
class EnvironmentWorker(QThread):
    ...
    def __init__(self, project_name: str, packages: list[str]):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            verify_result = wrangler.verify_imports(self.project_name, self.packages)
            ...
            if verify_result.success:
                ...
                self.finished.emit(verify_result)
...
class PackWorkspace(QWidget):
    ...
    def setup_ui(self):
        ...
        main_layout.addWidget(deps_section)
        ...
        main_layout.addWidget(env_section)
        ...
        main_layout.addWidget(build_section)
    ...
    def on_setup_environment(self):
        ...
        packages = list(self.scan_result.third_party) if self.scan_result else []
        
        # Always include pyinstaller - it's needed for building
        if 'pyinstaller' not in [p.lower() for p in packages]:
        ...
        # Setup progress bar
        self.env_progress.setMaximum(len(packages) if packages else 0)
        ...
        # Start worker
        self.env_worker = EnvironmentWorker(self.project.name, packages)
    ...
    def on_build(self):
        ...
        # Check for icon (look for converted icons first)
        icon_path = None
        if self.icon_path:
            import platform
            if platform.system() == "Darwin":
                # Look for .icns version
                icns_path = self.icon_path.with_suffix('.icns')
                if icns_path.exists():
                    icon_path = icns_path
                elif self.icon_path.suffix.lower() == '.icns':
                    icon_path = self.icon_path
                else:
                    icon_path = self.icon_path  # Use PNG, PyInstaller may convert
            elif platform.system() == "Windows":
                # Look for .ico version
                ico_path = self.icon_path.with_suffix('.ico')
                if ico_path.exists():
                    icon_path = ico_path
                elif self.icon_path.suffix.lower() == '.ico':
                    icon_path = self.icon_path
                else:
                    icon_path = self.icon_path  # Use PNG
    ...
    def on_open_build_folder(self):
        ...
        orchestrator = BuildOrchestrator()
        if not orchestrator.open_build_folder(self.project.path):
            # Folder might not exist yet
            build_dir = self.project.path / "TPC Builds"
            QMessageBox.information(
                self,
                "Build Folder",
                f"Build folder location:\n\n{build_dir}\n\n"
                "No builds yet - click Build to create one!"
            )
